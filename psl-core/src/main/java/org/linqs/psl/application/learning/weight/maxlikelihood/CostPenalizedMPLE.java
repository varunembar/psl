 ...
 */
public class CostPenalizedMPLE extends VotedPerceptron {
	 ...
	 * @see ConfigManager
	 */
	public static final String CONFIG_PREFIX = "costmple";
	 ...
	 * @param config
	 */
	public CostPenalizedMPLE(Model model, Database rvDB, Database observedDB, ConfigBundle config) {
	...
	protected void computeExpectedIncompatibility() {
		...
		for (RandomVariableAtom atom : rvAtomSet) {
			...
			for (int j = 0; j < s.length; j++) {
				...
				double exp = Math.exp(sum + costFunction);
				/* Add to partition */
