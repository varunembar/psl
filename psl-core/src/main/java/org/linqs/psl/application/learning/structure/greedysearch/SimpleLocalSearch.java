...
public class SimpleLocalSearch extends StructureSelectionApplication {
	...
	public static final double INIT_RULE_WEIGHT_DEFAULT = 2.0;
	...
	public SimpleLocalSearch(Model model, Database rvDB, Database observedDB, ConfigBundle config, Set<Rule> candidateRules) {
	...
	protected void doLearn() {
		...
		while(Math.abs(previousBestScore - bestScore) > tolerance && iter < maxIterations){
			...
			for(Rule candRule : candidateRuleSet){
				...
				double score = 0.0;
				try{
					this.resetRuleWeights(model.getRules());
					mle.learn();
	...
	private void resetRuleWeights(List<Rule> rules){
		for(Rule rule : rules){
			((WeightedRule)rule).setWeight(initRuleWeight);
		}
	}
